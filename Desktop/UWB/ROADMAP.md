...
────────────────────────────────────────────────────
Phase A    안전 정확성          🔴 최우선  ✅ 완료·하드웨어 검증 (C1·C2, C5는 교정 SOP)
...
> **상태 (2026-06-24):** C1(비대칭 median)·C2(태그 RX 타임아웃) 펌웨어 반영 및 실기기 동작 확인 완료.
> C5(안테나 교정)는 2점 교정 SOP 코드 주석화, **실측 교정은 별도 진행 필요**.
> 추가: 프레임 구조(12/20-byte, 'U'/'W' 마커, MSG_TYPE_IDX=9)·SPI 7MHz·`dwt_softreset()` 수정으로 ranging 동작.
